#!/usr/bin/env python3
"""
Side-branch calibration test: compare channel-192 time with MCP trigger_time.

This script extracts ch192 time and MCP trigger_time event-by-event,
produces a 2D scatter with linear fit, and histograms of the residuals.

Calibration approach:
  1. Linear fit: trigger_time = m * ch192_aligned + b
  2. Compute residuals from linear fit
  3. Fit poly-N on residual vs trigger_time (iterative ±3 MAD outlier removal)
  4. Calibrate: ch192_cal = ch192_aligned + poly(trigger_time) / m

Reuses functions from the main bar-analysis workflow:
  - bar_helpers.find_data_tree, gauss
  - bar_plotting.plot_t_diff  (1D histogram + Gaussian fit)

Example:
  python3 ch192_vs_trigger.py /path/to/4405_*_e.root --mcp-internal-dt-cut
"""
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Side-branch test: ch192 vs MCP trigger_time comparison"
    )
    ...
    # output
    p.add_argument("--out-prefix", default="ch192_vs_trig",
                   help="Prefix for all output plot filenames")
    p.add_argument("--nbins", type=int, default=120)
    p.add_argument("--fit-degree", type=int, default=3,
                   help="Polynomial degree for residual calibration fit (default: 3)")
    p.add_argument("--cal-bins", type=int, default=20,
                   help="Number of bins for binned residual correction (default: 20)")
...
# ──────────────────────────────────────────────────────────────────
# MCP map builder  (simplified from bar_processing._build_mcp_maps)
# ──────────────────────────────────────────────────────────────────

def build_mcp_map(root_file, cfg):
    """Return dict: event_index -> (trigger_time, peak_time).

    Only requires trigger_time to be finite.
    Amplitude and internal-dt cuts are applied only if explicitly enabled.
    """
    mcp_tree_name = cfg["mcp_tree"]
    if mcp_tree_name not in root_file:
        log(f"MCP tree '{mcp_tree_name}' not found")
        return {}

    t = root_file[mcp_tree_name]
    mcp_idx = t[cfg["mcp_index"]].array(library="np")
    trig_time = t[cfg["mcp_trigger_time"]].array(library="np")
    peak_time = t[cfg["mcp_peak_time"]].array(library="np")

    # optional branches (only loaded if cuts are requested)
    def _safe_array(branch):
        try:
            return t[branch].array(library="np")
        except Exception:
            return np.full(len(mcp_idx), np.nan)

    keep = np.isfinite(trig_time)
    n_before = int(keep.sum())

    # amplitude cuts (only if user explicitly sets min/max)
    amp_min = cfg.get("mcp_peak_amp_min")
    amp_max = cfg.get("mcp_peak_amp_max")
    if amp_min is not None or amp_max is not None:
        peak_amp = _safe_array(cfg["mcp_peak_amp"])
        if amp_min is not None:
            keep &= np.isfinite(peak_amp) & (peak_amp >= amp_min)
        if amp_max is not None:
            keep &= np.isfinite(peak_amp) & (peak_amp <= amp_max)

    # internal dt cut (only if user passes --mcp-internal-dt-cut)
    if cfg.get("mcp_internal_dt_cut", False):
        peak_time = _safe_array(cfg["mcp_peak_time"])
        internal_keep, dt_summary = _mcp_internal_dt_selector(
            peak_time[keep], trig_time[keep], cfg.get("mcp_internal_dt_nmad", 3.0)
        )
        filtered = np.zeros(len(mcp_idx), dtype=bool)
        filtered[keep] = internal_keep
        keep = filtered
        if dt_summary:
            log(f"MCP internal dt cut [{dt_summary['method']}] "
                f"median={dt_summary['median']:.3f} ps, "
                f"half-window={dt_summary['width']:.3f} ps, "
    ...
    for j in range(len(mcp_idx)):
        ...
        try:
            evt = int(mcp_idx[j])
            tt = float(trig_time[j])
            pt = float(peak_time[j]) if j < len(peak_time) else float('nan')
        except Exception:
            continue
        mcp_map[evt] = (tt, pt)

    log(f"MCP map: {len(mcp_map)} valid entries (from {len(mcp_idx)} total, "
        f"{n_before} had finite trigger_time)")
    return mcp_map


# ──────────────────────────────────────────────────────────────────
# Fast (vectorised) extraction of ch192 time per event
# ──────────────────────────────────────────────────────────────────

def extract_ch_times(root_file, cfg, mcp_map):
    """
    Return parallel arrays:
      ch_time, mcp_trig_time
    for events where both ch and MCP trigger_time are present.
    """
    tree_name = find_data_tree(root_file)
    if tree_name is None:
        log("No data tree found")
        return {k: np.array([]) for k in
                ["ch_time", "mcp_trig_time", "mcp_peak_time", "event_idx", "val_time",
                 "val_bar_ok", "val_energy"]}

    tree = root_file[tree_name]
    ch_target = cfg["channel"]
    val_channel = cfg.get("val_channel", 137)
    branches = [cfg["branch_channel"], cfg["branch_time"]]
    branch_energy = cfg.get("branch_energy", "energy")
    if branch_energy in tree.keys():
    ...
    # Diagnostic: compare data tree size vs MCP index range
    mcp_indices = sorted(mcp_map.keys())
    log(f"Data tree: '{tree_name}', entries={tree.num_entries}, "
        f"reading up to {max_entries}")
    log(f"MCP index range: [{min(mcp_indices)}, {max(mcp_indices)}], "
        f"count={len(mcp_indices)}")
    n_out_of_range = sum(1 for idx in mcp_indices if idx >= max_entries)
    if n_out_of_range > 0:
        log(f"WARNING: {n_out_of_range} MCP indices >= data tree entries "
            f"(will never match!)")
    ...
    counters = {"total_data": 0, "has_mcp": 0, "no_ch": 0, "kept": 0}
    ...
    for start in range(0, max_entries, step):
        ...
        for local_i in range(n_evt):
            ...
            # Also extract validation channel time
            val_positions = np.where(ch_list == val_channel)[0]
            if len(val_positions) > 0:
                val_pos = val_positions[0]
                if val_pos < len(t_list):
                    val_t = float(t_list[val_pos])
                else:
                    val_t = float('nan')
            else:
                val_t = float('nan')
            out["val_time"].append(val_t)

            # Check if event has ONLY the 4 bar channels + ch192
            required_channels = {137, 150, 234, 243, ch_target}
            ch_set = set(ch_list.tolist())
            out["val_bar_ok"].append(ch_set == required_channels)

            # Extract val channel energy
            if has_energy and len(val_positions) > 0:
                e_arr = ak.to_numpy(arrays[branch_energy][local_i])
                if val_pos < len(e_arr):
                    out["val_energy"].append(float(e_arr[val_pos]))
                else:
                    out["val_energy"].append(float('nan'))
            else:
                out["val_energy"].append(float('nan'))
            counters["kept"] += 1

    log(f"Data tree entries: {counters['total_data']}, "
        f"with MCP match: {counters['has_mcp']}, "
        f"ch{ch_target} missing: {counters['no_ch']}, "
...
def plot_scatter_with_fit(x, y, out_path, xlabel, ylabel, title):
    """2D scatter + linear fit."""
    plt.figure(figsize=(7, 5))
    plt.scatter(x, y, s=4, alpha=0.4, color="steelblue")

    if len(x) >= 2:
        # center for numerical stability
        x0, y0 = float(np.mean(x)), float(np.mean(y))
        m, b = np.polyfit(x - x0, y - y0, 1)
        x_line = np.linspace(x.min(), x.max(), 200)
        y_line = m * (x_line - x0) + b + y0
        plt.plot(x_line, y_line, color="red", linewidth=2,
                 label=f"Linear fit: slope={m:.6f}")
        plt.legend()
    ...
    return m if len(x) >= 2 else None


def plot_residual_hist(residuals, out_path, xlabel, title, nbins=120):
    """Histogram of residuals with Gaussian fit (reuses plot_t_diff)."""
    return plot_t_diff(
        residuals.tolist(), out_path, title,
        nbins=nbins, xlabel=xlabel, color="teal"
    )


def _gauss_fit_hist(ax, data, n_bins, color, xlabel, label_prefix, n_raw):
    """Plot histogram on ax, overlay Gaussian fit, return (mu, sigma) or None."""
    counts, bin_edges, _ = ax.hist(
        data, bins=n_bins, color=color, alpha=0.7,
    ...
    mu_est, sig_est = float(np.mean(data)), float(np.std(data))
    amp_est = float(counts.max())
    try:
        popt, _ = curve_fit(gauss, bin_centers, counts,
                            p0=[amp_est, mu_est, sig_est],
                            maxfev=5000)
        amp_f, mu_f, sig_f = popt
        sig_f = abs(sig_f)
        x_fit = np.linspace(bin_edges[0], bin_edges[-1], 300)
        ax.plot(x_fit, gauss(x_fit, *popt), color="red", linewidth=2,
                label=f"Gauss: μ={mu_f:.1f}, σ={sig_f:.1f} ps")
        ax.legend(fontsize=8)
        ax.set_title(f"{label_prefix} (±3MAD, N={len(data)}/{n_raw})\n"
                     f"Gauss: σ={sig_f:.1f} ps, μ={mu_f:.1f} ps")
        return mu_f, sig_f
    except Exception:
        ax.set_title(f"{label_prefix} (±3MAD, N={len(data)}/{n_raw})\n"
                     f"σ≈{sig_est:.1f} ps, μ≈{mu_est:.1f} ps (fit failed)")
        return None
...
def detect_segments(mcp_trig, ch_time):
    """
    Detect data segments by finding large backward jumps in mcp_trig.
    Returns a list of boolean masks, one per segment.
    """
    order = np.argsort(ch_time)
    trig_sorted = mcp_trig[order]
    diffs = np.diff(trig_sorted)
    median_step = np.median(np.abs(diffs[diffs != 0])) if np.any(diffs != 0) else 1.0
    threshold = -50.0 * max(median_step, 1e6)
    reset_indices = np.where(diffs < threshold)[0] + 1

    labels_sorted = np.zeros(len(ch_time), dtype=int)
    seg_id = 0
    for idx in reset_indices:
        seg_id += 1
        labels_sorted[idx:] = seg_id

    labels = np.empty(len(ch_time), dtype=int)
    labels[order] = labels_sorted

    n_seg = seg_id + 1
    masks = [labels == s for s in range(n_seg)]
    log(f"Detected {n_seg} segments (sizes: {[int(m.sum()) for m in masks]})")
    return masks


def _process_one_file(path, cfg):
    """Process a single ROOT file, return data dict or None."""
    f = uproot.open(path)
    mcp_map = build_mcp_map(f, cfg)
    if not mcp_map:
        return None
    return extract_ch_times(f, cfg, mcp_map)


# ──────────────────────────────────────────────────────────────────
# Per-file analysis (segments, residual-based calibration, plots)
# ──────────────────────────────────────────────────────────────────

def _per_file_analysis(ch_time, mcp_trig, mcp_peak, val_time, val_bar_ok,
                       val_energy, prefix, ch, val_ch, nbins, fit_degree,
                       cal_bins=20):
    """
    Per-file segment analysis with residual-based polynomial calibration.

    Calibration approach per segment:
      1. Linear fit: trigger = m * ch192_aligned + b
      2. Residual = trigger - (m * ch192_aligned + b)
      3. Fit poly-N on residual vs trigger_time (iterative ±3 MAD)
      4. ch192_cal = ch192_aligned + poly(trigger_time) / m

    All per-segment plots are saved using the given prefix.
    Returns dict with calibrated data and summary info.
    """
    ...
    plt.title(f"MCP trigger_time vs Channel {ch} (aligned)  ({n_seg} segments)")
    ...
    # ── Per-segment residual analysis + calibration ──
    ...
    for s, mask in enumerate(seg_masks):
        ...
        sig = plot_t_diff(
            ...
            f"Residuals (pre-cal): Seg {s+1}  (slope={m:.6f}, N={len(clean)}/{len(resid)})",
        ...
        # ── Binned residual correction + plot ──
        # Compute median residual per trigger_time bin, interpolate linearly
        clean_mask = fit_idx.copy()
        n_cal_bins = cal_bins
        bin_correction = None

        if clean_mask.sum() >= n_cal_bins:
            trig_clean = y_s[clean_mask]
            resid_clean = resid[clean_mask]
            bin_edges_cal = np.linspace(trig_clean.min(), trig_clean.max(), n_cal_bins + 1)
            bin_centers = 0.5 * (bin_edges_cal[:-1] + bin_edges_cal[1:])
            bin_medians = np.full(n_cal_bins, np.nan)

            for ib in range(n_cal_bins):
                in_bin = (trig_clean >= bin_edges_cal[ib]) & (trig_clean < bin_edges_cal[ib + 1])
                if ib == n_cal_bins - 1:  # include right edge for last bin
                    in_bin |= (trig_clean == bin_edges_cal[ib + 1])
                if in_bin.sum() >= 3:
                    bin_medians[ib] = np.median(resid_clean[in_bin])

            # Fill NaN bins with nearest valid neighbor
            valid = np.isfinite(bin_medians)
            if valid.sum() >= 2:
                from scipy.interpolate import interp1d
                interp_func = interp1d(bin_centers[valid], bin_medians[valid],
                                       kind="linear", fill_value="extrapolate")
                bin_correction = interp_func  # callable
                log(f"Segment {s+1}: binned correction with {n_cal_bins} bins, "
                    f"{valid.sum()} valid")

        # Plot: residual vs trigger_time with binned correction overlay
        fig, ax = plt.subplots(figsize=(10, 5))
        ax.scatter(y_s, resid, s=2, alpha=0.3, color="steelblue")
        ax.axhline(0, color="grey", linewidth=1, linestyle="--")
        if bin_correction is not None:
            plot_x = np.linspace(y_s.min(), y_s.max(), 300)
            ax.plot(plot_x, bin_correction(plot_x),
                    color="red", linewidth=2,
                    label=f"Binned median ({n_cal_bins} bins)")
            # Also plot bin centers
            if 'bin_centers' in dir() and 'bin_medians' in dir():
                valid_bc = np.isfinite(bin_medians)
                ax.scatter(bin_centers[valid_bc], bin_medians[valid_bc],
                           color="red", s=30, zorder=5, marker="s")
            ax.legend(fontsize=8)
        ax.set_xlabel("Trigger time (ps)")
        ax.set_ylabel("Residual (ps)")
        ax.set_title(f"Seg {s+1}: residual vs trigger_time  (N={len(resid)})")
        ax.grid(True, alpha=0.3)
        fig.tight_layout()
        ...
        # Plot: fit-only events (clean subset with y-axis clipped)
        ...
        ax.axhline(0, color="grey", linewidth=1, linestyle="--")
        if bin_correction is not None:
            plot_x = np.linspace(y_fit.min(), y_fit.max(), 300)
            ax.plot(plot_x, bin_correction(plot_x),
                    color="red", linewidth=2,
                    label=f"Binned median ({n_cal_bins} bins)")
            if 'bin_centers' in dir() and 'bin_medians' in dir():
        ...
        ax.set_xlabel("Trigger time (ps)")
        ax.set_ylabel("Residual (ps)")
        ax.set_title(f"Seg {s+1}: fit-only events  (N={n_clean})")
        ax.grid(True, alpha=0.3)
        fig.tight_layout()
        ...
        # ── Calibrate ch192 (binned correction) ──
        if bin_correction is not None:
            correction = bin_correction(y_s)
            ch_time_cal[mask] = x_s - correction

            # Calibrated residuals (should be flat)
            resid_cal = resid - bin_correction(y_s)
            ...
            sig_cal = plot_t_diff(
                ...
                f"Residuals (post-cal1): Seg {s+1} (±3MAD, N={len(clean_cal)}/{len(resid_cal)})",
                nbins=min(nbins, max(30, len(clean_cal) // 15)),
                xlabel="Calibrated residual (ps)"
            )
            seg_sigmas_cal.append(sig_cal)
            if sig_cal is not None:
                log(f"Segment {s+1} post-cal1 residual sigma = {sig_cal:.2f} ps")
            ...
            ax.set_title(f"Seg {s+1}: calibrated residual vs trigger_time  (N={len(resid_cal)})")
            ax.grid(True, alpha=0.3)
            fig.tight_layout()
            ...
            ax.set_xlabel("Trigger time (ps)")
            ax.set_ylabel("Calibrated residual (ps)")
            ax.set_title(f"Seg {s+1}: cal residual fit-only  (N={int(clean_mask.sum())})")
            ax.grid(True, alpha=0.3)
            fig.tight_layout()
    ...
    for s, mask in enumerate(seg_masks):
        ...
        if len(x_s) >= 2:
            ...
            ax.legend(fontsize=8)
        ax.set_xlabel("MCP trigger_time (ps)")
        ax.set_ylabel(f"Channel {ch} cal1 (ps)")
        ax.set_title(f"Segment {s+1} (cal1, {len(x_s)} events)")
        ax.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(f"{prefix}_scatter_segments_cal1.png", dpi=150)
    plt.close()
    log(f"Saved: {prefix}_scatter_segments_cal1.png")
    ...
    if all_resid:
        ...
        ax.set_ylabel("Counts")
        ax.set_title(f"Pre-cal residuals: {n_seg} segments")
        ax.legend(fontsize=8)
        ax.grid(True, alpha=0.3)
        fig.tight_layout()
    ...
    if all_resid_cal:
        ...
        ax.set_ylabel("Counts")
        ax.set_title(f"Post-cal residuals: {n_seg} segments")
        ax.legend(fontsize=8)
        ax.grid(True, alpha=0.3)
        fig.tight_layout()
    ...
    if val_ok.sum() >= 10:
        ...
        res_a = _gauss_fit_hist(ax2, ca, nb, "tab:green",
                                f"[ch{val_ch}−ch{ch}_cal] − [mcp_peak−trigger] (ps)",
                                "After cal", len(delta_after))

        fig.suptitle(f"Validation: [ch{val_ch} − ch{ch}] − [mcp_peak − trigger]", fontsize=13)
        fig.tight_layout()
...
def main():
    ...
    per_file_data = []  # list of (file_path, data_dict)
    ...
    for file_path, data in per_file_data:
        ...
        n_f = len(fch)
        log(f"\n{'='*60}")
        log(f"  File: {basename}  ({n_f} events)")
        ...
        result = _per_file_analysis(
            fch, fmcp, fmpk, fval, fbar, feng,
            file_prefix, ch, val_ch, nbins, args.fit_degree,
    ...
    log(f"\nTotal matched events across all files: {n}")
    if n < 2:
    ...
    if all_resid_global:
        ...
        ax.set_xlabel("Residual (ps)")
        ax.set_ylabel("Counts")
        ax.set_title(f"Pre-cal residuals: all {len(all_resid_global)} segments combined")
        ax.legend(fontsize=8)
        ax.grid(True, alpha=0.3)
        fig.tight_layout()
        fig.savefig(f"{prefix}_residuals_combined.png", dpi=150)
    ...
    if all_resid_cal_global:
        ...
        ax.set_xlabel("Calibrated residual (ps)")
        ax.set_ylabel("Counts")
        ax.set_title(f"Post-cal residuals: all segments combined")
        ax.legend(fontsize=8)
        ax.grid(True, alpha=0.3)
        fig.tight_layout()
        fig.savefig(f"{prefix}_residuals_combined_cal.png", dpi=150)
    ...
    sig_wrap = plot_t_diff(
        ...
        f"Δt = ch{ch} − trigger_time (wrapped ±3125 ps)",
    ...
    if val_ok.sum() >= 10:
        ...
        delta_before = (val_time[val_ok] - ch_time[val_ok]) - (mcp_peak[val_ok] - mcp_trig[val_ok])
        # After calibration (ch192_cal recovered to original frame)
        delta_after = (val_time[val_ok] - ch_time_cal_orig[val_ok]) - (mcp_peak[val_ok] - mcp_trig[val_ok])
        ...
        log(f"After cal:  σ={sig_a:.2f} ps")

    # ────────────────────────────────────────────────────
    # Summary
    # ────────────────────────────────────────────────────
    print("\n" + "=" * 55)
    print(f"  ch{ch} vs MCP trigger_time  —  Summary")
    print("=" * 55)
    ...
    for fname, n_seg, n_evt, slopes, sigmas, sigmas_cal in all_file_summary:
        ...
        for s in range(n_seg):
            ...
            if sig_c is not None:
                print(f"      Post-cal resid σ   : {sig_c:.2f} ps")
    print("-" * 55)
    if sig_wrap is not None:
