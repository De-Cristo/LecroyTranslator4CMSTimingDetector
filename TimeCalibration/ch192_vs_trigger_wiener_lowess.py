...
def _per_file_analysis_lowess(ch_time, mcp_trig, mcp_peak, val_time, val_bar_ok,
    ...
    for s, mask in enumerate(seg_masks):
        ...
        ax.axhline(0, color="grey", linewidth=1, linestyle="--")
        if lowess_available:
            ax.plot(y_s[sort_order_all], lowess_resid[sort_order_all], color="red", linewidth=2,
                    label=f"LOWESS drift")
        ax.legend(fontsize=8)
        ax.set_xlabel("Trigger time (ps)")
...
def main():
    ...
    if val_ok.sum() >= 10:
        ...
        delta_before = (val_time[val_ok] - ch_time[val_ok]) - (mcp_peak[val_ok] - mcp_trig[val_ok])
        delta_wiener = (val_time[val_ok] - ch_time_wiener_orig[val_ok]) - (mcp_peak[val_ok] - mcp_trig[val_ok])
        delta_lowess = (val_time[val_ok] - ch_time_cal_lowess_orig[val_ok]) - (mcp_peak[val_ok] - mcp_trig[val_ok])
        delta_expected = (val_time[val_ok] - ch_time_expected_orig[val_ok]) - (mcp_peak[val_ok] - mcp_trig[val_ok])
        val_energy_ok = val_energy[val_ok]
        dt_before = val_time[val_ok] - ch_time[val_ok]
        dt_wiener = val_time[val_ok] - ch_time_wiener_orig[val_ok]
        dt_lowess = val_time[val_ok] - ch_time_cal_lowess_orig[val_ok]
        dt_expected = val_time[val_ok] - ch_time_expected_orig[val_ok]
        labels = ["Before cal", "Wiener", "LOWESS", "Expected ch192"]
        ...
        _save_inverse_energy_scatter(
            prefix, val_energy_ok, [dt_before, dt_wiener, dt_lowess, dt_expected], val_ch, ch,
            "[Wiener+LOWESS] Validation inverse-energy", labels
        )

        delta_before_walk = _apply_walk_correction(delta_before, val_energy_ok, f"{prefix}_walk_before", f"Walk Fit: Before cal (ch{val_ch})")
        delta_wiener_walk = _apply_walk_correction(delta_wiener, val_energy_ok, f"{prefix}_walk_wiener", f"Walk Fit: Wiener (ch{val_ch})")
        delta_lowess_walk = _apply_walk_correction(delta_lowess, val_energy_ok, f"{prefix}_walk_lowess", f"Walk Fit: LOWESS (ch{val_ch})")
        delta_expected_walk = _apply_walk_correction(delta_expected, val_energy_ok, f"{prefix}_walk_expected", f"Walk Fit: Expected ch192 (ch{val_ch})")

        clean_before = mad_clip(delta_before_walk)
        clean_wiener = mad_clip(delta_wiener_walk)
        clean_lowess = mad_clip(delta_lowess_walk)
        ...
        res_w = _gauss_fit_hist(ax2, clean_wiener, nb, "tab:orange",
                                f"[ch{val_ch}−ch{ch}_wiener] − MCP_walk (ps)",
                                "Wiener", len(delta_wiener_walk))
        res_l = _gauss_fit_hist(ax3, clean_lowess, nb, "tab:red",
                                f"[ch{val_ch}−ch{ch}_lowess] − MCP_walk (ps)",
                                "LOWESS", len(delta_lowess_walk))
        res_e = _gauss_fit_hist(ax4, clean_expected, nb, "tab:green",
                                f"[ch{val_ch}−ch{ch}_expected] − MCP_walk (ps)",
                                "Expected ch192", len(delta_expected_walk))
        fig.suptitle(f"[Wiener+LOWESS] Validation (Walk Corrected): ch{val_ch} vs ch{ch}",
                     fontsize=13)
        fig.tight_layout()
        fig.savefig(f"{prefix}_validation.png", dpi=150)
        plt.close(fig)
        ...
        sig_w = res_w[1] if res_w else np.std(clean_wiener)
        sig_l = res_l[1] if res_l else np.std(clean_lowess)
        sig_e = res_e[1] if res_e else np.std(clean_expected)
        log(f"Before cal: σ={sig_b:.2f} ps")
        log(f"Wiener: σ={sig_w:.2f} ps")
        log(f"LOWESS: σ={sig_l:.2f} ps")
        log(f"Expected ch192: σ={sig_e:.2f} ps")
