...
def main():
    ...
    ch_dumps, idx_dumps, time_dumps, ener_dumps = {}, {}, {}, {}
    with uproot.open(args.root) as f_in:
        ...
        local_idx = args.branch_idx if args.branch_idx else ("channelIdx" if "channelIdx" in keys_in else None)
        ...
        if local_idx and local_idx in keys_in: read_cols.append(local_idx)
        ...
        while entry_start < n_entries:
            ...
            if chunk_matches:
                ...
                for ev in chunk_matches:
                    ...
                    try: idx_arr = ak.to_list(arrs[local_idx][local_i]) if local_idx in read_cols else []
                    except: idx_arr = []
                    ...
                    idx_dumps[ev] = json.dumps(idx_arr)
    ...
    for row in matched_rows:
        ...
        row["channelIdx"] = idx_dumps.get(ev, "[]")
    ...
    if len(out_df) > 0:
        ...
        std_cols = ['entry', 'channelID', 'channelIdx', 'time', 'energy', 'mcp_index', 'mcp_peak_time', 'mcp_peak_amp', 'mcp_peak_phase', 'mcp_trigger_time']
